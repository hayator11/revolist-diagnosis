...
> 社長確定値を反映済み。診断URLも確定済み。本文の投稿（#01〜#11）は無改変。

| 項目 | 現状 | 状態・差し替え先 |
|---|---|---|
| 診断本番URL（プロフィールリンク） | `https://revo.onokun.com/diagnosis/entry?src=threads` | **確定・社長確認済み**（okurimono内カードのURL）。確定次第そのまま確定URLへ。**`?src=threads` は必ず維持** |
| アカウント名 | `@____hayator` | **確定** |
| 表示名 | `レボリスト診断｜11タイプで仲間を見つける` | **確定・社長指定**（`21-profile.md §1 ★確定版`） |
| bio | `21-profile.md §2 ★確定版` | **確定・社長指定**（先頭スローガン「世界一やさしい贈り物。」＋診断導線） |
